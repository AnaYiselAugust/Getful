...
// ----------------SIGN UP----------------------------
	o.signUp = function(user){
		var q = $q.defer();
		$http.post("http://localhost:3000/api/user/signUp", user).then(function(res){
			setToken(res.data);
			setUser();
			var user = o.getUser();
...
//------------FORGOT PASSWORD? SEND EMAIL--------------------------
	o.forgot = function(user) {
			var q = $q.defer() ;
			$http.post('http://localhost:3000/api/user/forgot', user).success(function(res) {
				q.resolve() ;
			}) ;
	...
	o.resetPassword = function(editedUser) {
		var q = $q.defer();
		$http.put('http://localhost:3000/api/user/resetPassword/' + editedUser.id, editedUser).success(
			function(res){
				q.resolve(res);
			});
			return q.promise;
